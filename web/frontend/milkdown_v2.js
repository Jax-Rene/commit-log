    ...
    const reAlignSlashMenu = (menu) => {
        try {
            ...
            editor.action((ctx) => {
                ...
                const pos =
                    typeof selection?.to === "number" ? selection.to : 0;
                const coords = view.coordsAtPos(Math.max(pos, 0));
                const parent =
                    menu.offsetParent || menu.parentElement || document.body;
                ...
                const scrollTop =
                    typeof parent.scrollTop === "number"
                        ...
                        : window.scrollY;
                const top =
                    (coords?.bottom ?? 0) - parentRect.top + scrollTop + 10;
    ...
    editor.config((ctx) => {
        ctx.update(uploadConfig.key, (prev) => {
            const fallbackUploader =
                prev && typeof prev.uploader === "function"
                    ? prev.uploader
                    : null;
            return {
                ...prev,
...
class PostDraftController {
    ...
    normalizePost(post) {
        ...
        if (
            ...
            typeof source.cover_url === "string"
        ) {
            source.CoverURL = source.cover_url;
        }
        if (
            source.CoverWidth === undefined &&
            source.cover_width !== undefined
        ) {
            source.CoverWidth = source.cover_width;
        }
        if (
            source.CoverHeight === undefined &&
            source.cover_height !== undefined
        ) {
    ...
    async resetToLatestPublication(options = {}) {
        ...
        this.postData.tags = clonedTags.map(
            (tag) => cloneValue(tag) || { ...tag },
        );
    ...
    buildPayload(content) {
        ...
        const markdown =
            typeof content === "string"
                ? content
                : coerceString(
                      pickProperty(post, ["Content", "content"], ""),
                      "",
                  );
    ...
    async publish(options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                const message =
                    data?.error || data?.message || "发布失败，请稍后重试";
                if (!silent) {
    ...
    async saveDraft(options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                const message =
                    data.error || data.message || "保存失败，请稍后重试";
            ...
            [toolbarKey]: {
                buildToolbar(builder) {
                    try {
                        if (
                            !builder ||
                            typeof builder.getGroup !== "function"
                        ) {
                            return;
                        }
                        ...
                        if (
                            items.some(
                                (item) => item && item.key === "inline-ai-chat",
                            )
                        ) {
                            return;
                        }
                        group.addItem("inline-ai-chat", {
                            ...
                            onRun: () => {
                                if (
                                    typeof inlineAIToolbarHandler === "function"
                                ) {
                                    inlineAIToolbarHandler();
                                } else {
                                    console.warn(
                                        "[milkdown] AI Chat 功能尚未就绪",
                                    );
                                }
                            },
                        });
